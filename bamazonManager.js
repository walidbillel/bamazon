...
function addInventory() {
  ...
  inquirer
    .prompt([
      {
        ...
        name: "productID",
        message:
          "Please Enter the product ID that you would like to add Inventory to?",
    ...
    .then(function (input) {
      ...
      connection.query(
        "SELECT * FROM products WHERE item_id = " + inventoryID,
        function (err, res) {
          if (err) throw err;

          // Adding up to the stock inventory
          var newInventoryQuantity =
            res[0].stock_quantity + parseFloat(inventoryWillingToAdd);
    ...
    .then(function (input) {
      ...
      connection.query(
        ...
        function (err, res) {
          if (err) throw err;
          console.log(
            "The New Product Was Successfully Added, You Will See it under Item ID: " +
              res.insertId +
              "\n"
          );
          setTimeout(displayInventory, 2000);
